...
at `/Volumes/team_b/landing/prod/postings/`. One copy of the
...
with **team-b** as the worked example. If that is your team, you only need your
...
secrets from Key Vault (commands are in `.env.example`; team-b names are shown
— replace `team-b` with your team letter if needed). Step 3 is only for the
...
```bash
# Team-b example — replace sthyffpteamb with your storage account if needed.
az storage blob list --account-name sthyffpteamb \
...
```sql
SELECT count(*) FROM read_files('/Volumes/team_b/landing/prod/postings',
                                format => 'json');
```
...
```
\d+ analytics_dev.fct_postings     -->  from team_b.dev_alex at 2026-08-13T11:21Z
```
...
```bash
az storage blob delete-batch --account-name sthyffpteamb --source dev \
