with
    ...
    transformed as (
        select
            ...
            case
                when salary_min is null and salary_max is null then false else true
            ...
            -- Flag rows where a bound is implausibly low or high for an annual NL
            -- salary
            case
                when salary_min is not null and salary_min < 5000
                then true
                when salary_max is not null and salary_max < 5000
                then true
                when salary_min is not null and salary_min > 300000
                then true
                when salary_max is not null and salary_max > 300000
                then true
                else false
            end as is_salary_outlier,

            -- Range width: how wide is the min-max spread (useful for spotting
            -- vague/broad postings)
            case
                when salary_min is not null and salary_max is not null
            ...
            case
                when salary_min is null and salary_max is null
                then 'unknown'
                when coalesce(salary_max, salary_min) < 30000
                then 'under_30k'
                when coalesce(salary_max, salary_min) between 30000 and 50000
                then '30k_50k'
                when coalesce(salary_max, salary_min) between 50001 and 75000
                then '50k_75k'
                when coalesce(salary_max, salary_min) between 75001 and 100000
                then '75k_100k'
                else 'over_100k'
            end as salary_band,
            ...
            case
                ...
                then
                    concat(
                        '€',
                        format_number(salary_min, 0),
                        ' - €',
                        format_number(salary_max, 0)
                    )
