with
    ...
    extract_contract as (
        select
            ...
            case
                when lower(title) rlike '\\b(parttime|part-time|bijbaan|deeltijd)\\b'
                then 'part_time'
    ...
    step2 as (
        select
            ...
            -- Example: "Schoonmaak werk voor 1 dagdeel in Doetinchem" -> "Schoonmaak
            -- werk"
            regexp_replace(
                trim(title),
    ...
    step3 as (
        select
            ...
            -- Example: "Operator 5 Ploegendienst" or "Operator 5-PLOEGEN" ->
            -- "Operator 5-ploegen"
            regexp_replace(
                title, '(?i)(\\d+)[\\s-]*ploegen(dienst)?', '$1-ploegen'
            ) as title
        from step2
    ),
    ...
    step4 as (
        select
            ...
            -- Example: "Elektromonteur/Storingsmonteur" -> "Elektromonteur /
            -- Storingsmonteur"
    ...
    step8 as (
        select
            ...
            -- Example: "Developer - Amsterdam" or "Teamleader regio Venlo" ->
    ...
    step9 as (
        select
            ...
            regexp_replace(
                regexp_replace(title, '\\.\\s+[A-Z].*$', ''), '\\s*[–-]+\\s*$', ''
            ) as title
        from step8
    ),
    ...
    step10 as (
        select
            ...
            -- colon
            -- Example: "Stageplek : Sales & Events bij KIT Amsterdam" -> "Sales &
            -- Events bij KIT Amsterdam"
            regexp_replace(
                title,
