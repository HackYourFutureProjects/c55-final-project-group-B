...
```text
landed 175 records, 1411925 bytes, to dev/<your-prefix>/postings/... on sthyffpteamb
Pipeline finished: 175 landed, 0 rejected, readable at /Volumes/team_b/landing/dev/<your-prefix>/postings
```
...
az storage blob list --account-name sthyffpteamb --container-name dev \
...
Next, to double-check, [open the team storage account in Azure Portal](https://portal.azure.com/#@hackyourfuture.nl/resource/subscriptions/1120c89d-2a5f-4a15-a582-2ea34f0bb5c3/resourceGroups/rg-hyf-fp-team-b/providers/Microsoft.Storage/storageAccounts/sthyffpteamb/containersList).
...
open your dev schema and table in the browser. Example (team-b, `dev_<yourname>`):

[See fct_postings in team_b.dev_<yourname> (Databricks Catalog Explorer)](https://adb-7405619530719547.7.azuredatabricks.net/explore/data/team_b/dev_<yourname>/fct_postings?o=7405619530719547) in databricks.
...
select * 
from team_b.dev_<yourname>.fct_postings
```
...
# Reads team_b.dev_<yourname>.fct_postings_enriched from Databricks and
...
```text
read 300 rows and 12 columns from team_b.dev_<yourname>.fct_postings_enriched
...
For **team-b**, the resource group is `rg-hyf-fp-team-b`, the registry is
`acrhyffpb`, and the dev job is `job-fp-ingest-dev` — all in `data/.env.example`.
...
> This should produce the url of the [ACA container](https://portal.azure.com/#@hackyourfuture.nl/resource/subscriptions/1120c89d-2a5f-4a15-a582-2ea34f0bb5c3/resourceGroups/rg-hyf-fp-team-b/providers/Microsoft.App/jobs/job-fp-ingest-dev/executionHistory) and you can view the execution history and logs of the ingestion job. You can also view the logs of the ingestion job in the portal.
...
LANDING_PATH=/Volumes/team_b/landing/dev/aca-dev/postings
```
...
| Airflow                  | your local Astro, `astro dev ps` URL      | `https://vm-hyf-team-b.westeurope.cloudapp.azure.com` |
...
There is also one shared admin login per team, `fp-airflow-admin-team-b` for
team-b, for maintainers rather than day-to-day use. Do not confuse either of
...
| What it does    | Triggers the `build` job, which builds one `pipeline` image and pushes it to `acrhyffpb` tagged both `latest` and with the commit SHA, then the `deploy-prod` job, which points `job-fp-ingest` — the production Container Apps job, a completely separate job from your `job-fp-ingest-dev` — at that SHA tag. |
...
- `LANDING_CONTAINER=prod` — the **prod ADLS container** on `sthyffpteamb`,
...
|                 |                                                                                                                                                                                                                                                                                                                             |
| --------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Where this runs | The prod Airflow UI, `[vm-hyf-team-b.westeurope.cloudapp.azure.com](https://vm-hyf-team-b.westeurope.cloudapp.azure.com)` — not your laptop, not Astro — running unattended on the schedule set in `pipeline_dag.py` (currently `0 9 * * *`, `Europe/Amsterdam` — see "The scheduled trigger, and how to change it" below). |
...
|                 |                                                                                                                                                |
| --------------- | ---------------------------------------------------------------------------------------------------------------------------------------------- |
| Where this runs | The **prod Airflow UI** at `[vm-hyf-team-b.westeurope.cloudapp.azure.com](https://vm-hyf-team-b.westeurope.cloudapp.azure.com)`, and Postgres. |
