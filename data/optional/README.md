...
after your team, for example `hyf-team-b`, so you can tell later which one to
revoke.
...
```bash
databricks secrets put-secret team_b openrouter-api-key
```

That opens an editor; paste the key, save, close. To check it without printing
it, `databricks secrets list-secrets team_b` shows the name and the time it
