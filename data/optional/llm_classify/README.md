...
|---|---|---|
| Notebook on laptop | `az login` → Key Vault `litellm-key-team-b` (or `LITELLM_API_KEY`) | `data/.env` with Databricks settings |
| `dbt run` on laptop | Databricks `team_b` / `litellm-api-key` | Enable model in `dbt_project.yml` |
