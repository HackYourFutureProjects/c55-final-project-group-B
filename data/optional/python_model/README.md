...
`int_postings_category` classifies titles with keyword rules from `tech_roles`.
...
Teachers provision `litellm-api-key` in Databricks secret scope `team_b`. Students
do not paste keys into the repo or `.env` for the dbt path.

| Runtime | Key source |
|---|---|
| `dbt build` (laptop, Astro, Airflow VM) | Databricks `team_b` / `litellm-api-key` |
...
from team_b.<your_dbt_schema>.fct_title_discipline
group by discipline
```
